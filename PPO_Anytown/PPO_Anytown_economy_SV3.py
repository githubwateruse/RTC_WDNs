# -*- coding: utf-8 -*-
"""
Created on Wed Nov  9 04:14:14 2022
...
def DEMAND_Observation(basedemand_scenario, period, ep_len):
    # 注意，这里的basedemand_scenario为 10元素组成的数组，每个数组包含19个节点需水量
    ...
    if period < ep_len:
        ...
        day_pattern_current = math.floor(period /24)
        
        for j in range (len(demand_group)):
            Demand_obs_current[j] = multiplier_current*basedemand_scenario[day_pattern_current][j]*1
        
    Demand_obs_next = np.zeros(len(demand_group))
    # period + 1 == ep_len时，代表period是末尾阶段，下一阶段的需水量都是0
    if period + 1 < ep_len:
        hour_pattern_next = (period + 1) % 24
        multiplier_next = Multipliers[hour_pattern_next]
        day_pattern_next = math.floor((period + 1) /24)
        
        for j in range (len(demand_group)):
            Demand_obs_next[j] = multiplier_next*basedemand_scenario[day_pattern_next][j]*1
...
def Objfunction(pump_solution, demand_obs_current, Tanklevel_Observation):
    ...
    # To extending the demand group observation, some sentences listed below may be useful 
    # period_demand_observation_1 = DEMAND_Observation(sets_Header, period_number + 1)
    # period_demand_observation_2 = DEMAND_Observation(sets_Header, period_number + 2)
    # Then concatenating the demand observation vectors
    
    # settings of variable speed pumps
    Timber = float(pump_solution) # 默认情况，什么都不做
    #Timber = np.sin(Timber* math.pi) # 情况一：正弦函数
    #Timber = 2 * np.sqrt(0.25 - (Timber-0.5)**2) # 情况二： 半圆
    #Timber = Timber ** 2 # 情况三：指数
...
path_train = Header_base + Header_for_train   #设置路径
...
def training_scenario_generation(scenario_number):
    ...
    # 针对每一回合(每一个ep)，都应该随机抽样出一个随机种子，用以提取相应的训练集(需水量数据序列)
    # scenario_number 的取值范围是 0-100之间的整数，每一个scenario_number代表一个时间周期为10天(即240h)的需水量数据序列
...
def Main_Loop_Training(State_vector, number, seed):
    ...
    parser.add_argument("--max_train_episodes", type=int, default=int(1500), help=" Maximum number of training episodes")
    ...
    workbook_curve = xlsxwriter.Workbook(Header_base + '/Convergence_curve_'+ State_vector +'.xlsx')
    ...
    for ep in range (args.max_train_episodes):
        ...
        for period in range(EP_LEN):
            ...
            Tnormal = Normal_T(period)
            
            Observation = np.append(np.append(np.append(Standard_tanklevel, Standard_demand_observation), Tnormal), action_pre)
            ...
            T_normal = Normal_T(period + 1)

            Observation_ = np.append(np.append(np.append(Nx_Sd_tanklevel_Observation, Nx_Sd_demand_observation), T_normal), action_current)
        ...
        agent.save(f"./" + "PPO_models" + "_" + State_vector + "/" + str(ep) + "_" + str(ep_r))
        EP_total_Reward = []
...
if __name__ == "__main__":
    ...
    # state vector
    State_vector = "SV3" # "SV1","SV2", "SV3", "SV4", "SV5"
    
    # For SV1, 3 elements: water levels of 3 tanks 
    # For SV2, 3 water levels + 19 node demand prediction
    # For SV4, 3 water levels + 19 node demand prediction + 时段T elements
    # For SV5, 3 water levels + 19 node demand prediction + 时段T elements (one-hot encoding)
