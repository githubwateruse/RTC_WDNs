# -*- coding: utf-8 -*-
"""
Created on Sat Nov 19 02:55:46 2022
...
def DEMAND_Observation(basedemand_scenario, period, ep_len):
    ...
    if period < ep_len:
        ...
        # 24 * 12 = 2016
        hour_pattern_current = period % 288
        week_pattern_current = math.floor(period/288)
...
def Objfunction(pump_solution, demand_obs_current, Tanklevel_Observation):
    ...
    tk.open(Ph, "D_town_mod_5m.inp", "D_town_mod_5m.rpt", "")
...
def Main_Loop_Training(State_vector, number, seed, uncertainty):
    ...
    parser.add_argument("--max_train_episodes", type=int, default=int(300), help=" Maximum number of training episodes")
    ...
    args.max_action = 1
    args.min_action = 0
    args.max_episode_steps = 24*12*150
    ...
    workbook_curve = xlsxwriter.Workbook(Header_base + '/Convergence_curve_'+ State_vector +'_5m.xlsx')
    ...
    for ep in range (args.max_train_episodes):
        ...
        agent.save(f"./" + "PPO_models" + "_" + State_vector + "_5m_" + uncertainty + "/" + str(ep) + "_" + str(ep_r/150))
        EP_total_Reward = []
